...
class ppFeatures:
    def __init__(self, image, bg, neighbors_r):
    ...
    def puncta_neighbors_tester(self, pidx):
        x1 = bboxes[pidx][1]
        y1 = bboxes[pidx][0]
        dy = np.round(puncta_properties_dictionaries[pidx].get('centroid-0'))
        dx = np.round(puncta_properties_dictionaries[pidx].get('centroid-1'))
        neighbor_ids = list(distanceslist[pidx].keys())
        ...
        for neighbor in neighbor_ids:
            x1_neighbor = bboxes[neighbor][1]
            y1_neighbor = bboxes[neighbor][0]
            dx_neighbor = np.round(puncta_properties_dictionaries[neighbor].get('centroid-1'))
            dy_neighbor = np.round(puncta_properties_dictionaries[neighbor].get('centroid-0'))
            ...
            plt.text(xvals[1] - 0.050, yvals[1]-0.25, str(np.round(distanceslist[pidx][neighbor], 2)), fontsize = 3)
