# -*- coding: utf-8 -*-
"""
Created on Thu Oct  6 16:38:55 2022
...
def bgsub(img):
    return(restoration.rolling_ball(img, radius=200))


def allFeats(img, mask): #include if using Method 2: , indx, isGFP):
    ...
    #Subsnippet for use in Method 2:
    # if isGFP == True:
    #     np.savetxt('GFP_finaldf_' + str(indx) + '.txt', finaldf)
    # elif isGFP == False:
    #     np.savetxt('TRITC_finaldf_' + str(indx) + '.txt', finaldf)
    
    return(finaldf) #Comment out if using Method 2

@jit(target_backend='cuda')
def allFeatswithGPU(img, mask): #include if using Method 2: , indx, isGFP):
